 ...
 */
static int spi_write_command(spi_device_t *spi_device, char command, char address, char value, char *result)
{
...
static ssize_t fop_slotcar_write(struct file *filp, const char *buff, size_t len, loff_t *offp)
{
  ...
  if(spi_write_command(data->spi_device, SPI_WRITE | SPI_EXTERNAL, *offp, data_to_write, &result) < 0) {
...
static ssize_t fop_slotcar_read(struct file *filp, char __user *buff, size_t count, loff_t *offp)
{
  ...
  char data_read;
  if(spi_send_command(data->spi_device, SPI_READ | SPI_EXTERNAL, *offp, SPI_TRANSFER_WAIT_FOR_READ, &data_read) < 0) {
    LOG_ERROR("Failed to read data");
    LOG_EXIT();
    return 0;
  }
  LOG_DEBUG("Read: %d-%c on memory address %lld", data_read, data_read, *offp);
